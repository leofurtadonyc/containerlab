...
- serves REST API to `app-web`
- exposes `/metrics` for Prometheus

## Current status
Initial backend skeleton exists with a FastAPI application entrypoint, a typed health endpoint, minimal configuration scaffolding, a placeholder metrics endpoint, package structure for routers, services, repositories, models, schemas, integrations, adapters, metrics, and config, plus Alembic migration scaffolding for future schema evolution and a bounded ODL integration skeleton location.

## Planned evolution
- read-only inventory, topology, and policy-oriented APIs
