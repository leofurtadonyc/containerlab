...
- `/api/v1/maintenance-preview` exposes **Maintenance Preview v1** (`maintenance_preview_v1`): reuse-only assembly of related-policies, embedded failure-impact v1, Service Explorer–style groupings over the related-policy subset, topology coverage echo, and navigation pointers—not simulation, safe-to-change scoring, or maintenance approval (**404** / **422** for unknown subject or selector mistakes)
- `/api/v1/reports/service-impact`, `/api/v1/reports/policy-impact`, and `/api/v1/reports/maintenance-impact` expose **Impact Report v1** (`impact_report_v1`): composed packaging of existing Service Explorer detail, policy dossier, or Maintenance Preview assemblies for operator communication—optional **`format=markdown`**; **not** compliance, validation authority, or approval (**404** / **422** per nested identity rules)
...
- `/api/v1/policies/{policy_id}/dossier` exposes a composed **policy dossier** read model (`policy_dossier_v1`): nested path-analysis, topology-impact, evidence timeline, evidence delta, inventory policy row, navigation hints, and merged caveats—reuse-only assembly from existing Phase 2 contracts; **404** when the policy id is absent from the current inventory list—not dataplane proof or workflow authority
