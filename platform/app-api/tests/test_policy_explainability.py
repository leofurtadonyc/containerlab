"""Contract tests for GET /api/v1/policies/{policy_id}/explainability."""
...
def test_policy_explainability_not_found(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_live_policy_and_topology(monkeypatch)
    response = client.get("/api/v1/policies/does-not-exist/explainability")
    assert response.status_code == 404


def test_policy_explainability_present_contract_and_nested_ids(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_live_policy_and_topology(monkeypatch)
    pid = "PE1:static_local:192.0.2.11:100"
    response = client.get(f"/api/v1/policies/{pid}/explainability")
    assert response.status_code == 200
    payload = response.json()
    assert payload["contract_id"] == "policy_explainability_workspace_v1"
    ...
    assert payload["navigation_targets"]["investigation_shell_params"]["inv_from"] == "policy_explainability"
    assert payload["navigation_targets"]["service_explorer_shell_params"]["service_id"] == f"policy:{pid}"
    assert payload["path_explanation_summary"]
    assert isinstance(payload["candidate_path_rollups"], list)
    assert payload["unknown_candidate_posture"] in ("none", "partial", "full")
    assert payload["sparse_signals"]["topology_naming_alignment_unknown"] is False
    assert payload["merged_caveats"]


def test_policy_explainability_sparse_topology_impact(monkeypatch: pytest.MonkeyPatch) -> None:
    """Empty topology-impact items: honest sparse flag and merged caveat."""
    _stub_live_policy_and_topology(monkeypatch)
    pid = "PE1:static_local:192.0.2.11:100"
    ...
    assert payload["sparse_signals"]["topology_naming_alignment_unknown"] is True
    merged = "\n".join(payload["merged_caveats"])
    assert "topology" in merged.lower() or "unknown" in merged.lower()


def test_policy_explainability_unknown_candidate_hints(monkeypatch: pytest.MonkeyPatch) -> None:
    """All candidate summaries unknown => full posture and honesty caveat."""
    _stub_live_policy_and_topology(monkeypatch)
    pid = "PE1:static_local:192.0.2.11:100"

    from app_api.services.path_analysis import build_policy_path_analysis_response as real_path

    def _all_unknown_paths(policy_id: str):
        out = real_path(policy_id)
        if out is None:
            return None
        summaries = [
            s.model_copy(update={"path_state": "unknown", "last_recorded_path_state": "unknown"})
            for s in out.candidate_path_summaries
        ]
        if not summaries:
            return out
        return out.model_copy(update={"candidate_path_summaries": summaries})

    monkeypatch.setattr(
        "app_api.services.policy_explainability.build_policy_path_analysis_response",
        _all_unknown_paths,
    )
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["unknown_candidate_posture"] == "full"
    for r in payload["candidate_path_rollups"]:
        assert r["signal"] == "unknown_signal"
    merged = "\n".join(payload["merged_caveats"])
    assert "incomplete" in merged.lower() or "ambiguous" in merged.lower()


def test_policy_explainability_comparison_not_ready_merges_caveat(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_live_policy_and_topology(monkeypatch)
    monkeypatch.setattr(
    ...
    assert payload["sparse_signals"]["evidence_delta_not_ready"] is True
    merged = "\n".join(payload["merged_caveats"])
    assert "comparison_status" in merged or "no_comparable_anchor" in merged


def test_policy_explainability_path_analysis_caveat_propagates(monkeypatch: pytest.MonkeyPatch) -> None:
