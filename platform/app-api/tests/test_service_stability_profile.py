"""Contract tests for GET /api/v1/services/{service_id}/stability-profile."""
...
def test_service_stability_profile_not_found(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_live_policy_and_topology(monkeypatch)
    response = client.get("/api/v1/services/does-not-exist:foo/stability-profile")
    assert response.status_code == 404


def test_service_stability_profile_contract_and_shape(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_live_policy_and_topology(monkeypatch)
    pid = "PE1:static_local:192.0.2.11:100"
    sid = f"policy:{pid}"
    response = client.get(f"/api/v1/services/{sid}/stability-profile")
    assert response.status_code == 200
    payload = response.json()
    assert payload["contract_id"] == "service_stability_profile_v1"
    ...
    assert "not_substitute_for_explorer_dossier_timeline_or_delta" in payload["safety_framing"][
        "explicit_non_claims"
    ]
...
def test_service_stability_profile_partial_nested_assembly(monkeypatch: pytest.MonkeyPatch) -> None:
    _stub_live_policy_and_topology(monkeypatch)
    pid = "PE1:static_local:192.0.2.11:100"
    sid = f"policy:{pid}"
    ...
    monkeypatch.setattr(
        "app_api.services.service_stability_profile.build_service_evidence_timeline_response",
        _boom,
    )
    response = client.get(f"/api/v1/services/{sid}/stability-profile")
    assert response.status_code == 200
    ...
    assert "service_evidence_timeline" in notes.lower() or "timeline" in notes.lower()
