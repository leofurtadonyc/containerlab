...
## Runtime details
- image: `platform-prometheus:0.1.0`, built from the pinned upstream `prom/prometheus:v2.54.1` base with a small startup validator
- ports: 9090
...
- startup posture: the local image now validates the mounted config, rules directories, and writable TSDB path, then runs `promtool check config` before starting Prometheus
