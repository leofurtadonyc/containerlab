...
class FakeGnmiClient:
    ...
    def get(self, *, path, encoding):
        del path, encoding
        host = self.target[0]
        device_name = "P1" if host.endswith(".109") else "PE1"
        return {
            "notification": [
...
def test_runtime_config_loads_live_nokia_targets() -> None:
    ...
    assert config.targets[0].name == "PE1"
    assert config.targets[1].name == "P1"
...
def test_metrics_endpoint_returns_inventory_operational_metrics(monkeypatch) -> None:
    ...
    assert "platform_gnmi_collector_inventory_targets 2" in response.text
    assert "platform_gnmi_collector_inventory_collection_success_total 2" in response.text
    assert "platform_gnmi_collector_inventory_normalization_failure_total 0" in response.text
    assert "platform_gnmi_collector_inventory_backend_ready_records 2" in response.text
...
def test_nokia_adapter_collects_live_inventory(monkeypatch) -> None:
    ...
    assert raw_record.raw_data["system_name"] == "PE1"
...
def test_inventory_flow_snapshot_prepares_live_backend_delivery(monkeypatch) -> None:
    ...
    assert snapshot.summary.target_count == 2
    assert snapshot.summary.collection_success_count == 2
    ...
    assert snapshot.summary.backend_ready_record_count == 2
