...
| [**Maintenance evidence workspace**](./maintenance-evidence-workspace-contract.md) (`maintenance_evidence_workspace_v1`) | Composed live **`GET /api/v1/maintenance-evidence-workspace`** for a topology maintenance subject | **Optional shell pivot** when **`topology_object`** is in briefing scope — **not** a briefing bundle member, **not** `evidence_export_v1`, **not** a substitute for **Impact report** or **Change safety case** |
| [**Maintenance window workspace**](./maintenance-window-workspace-contract.md) (`maintenance_window_workspace_v1`) | Composed live **`GET /api/v1/maintenance-window-workspace`** for bounded multi-subject rollups | **Optional shell pivot** when **`topology_object`** is in briefing scope (starts with that subject; operators add more on the workspace shell) — **not** a briefing bundle member, **not** `evidence_export_v1` |

**Implementation posture (v1):** Shipped as **`GET /api/v1/operator-briefing`** (**`operator_briefing_workspace_v1`**) in **`app-api`** (`schemas/operator_briefing.py`, `services/operator_briefing.py`, `routers/operator_briefing.py`)—a thin composition of **`cross_domain_delta_digest_v1`**, optional **policy** / **topology object** dossiers, **`evidence-pack` situation** assembly, and **`investigation-workspace` context**, with **`section_meta`**, **`merged_caveats`**, and **`recommended_pivots`**. The **WebUI** may still compose the same contracts client-side; this file defines **what the briefing may contain**, **section order**, **subject binding**, and **honesty limits**. Query parameters: **`sync_runs_limit`** (1–100), optional **`policy_id`**, **`topology_object`**, **`topology_object_kind`**, optional client echoes **`inv_from`**, **`global_search_q`**.
...
8. **Export actions framing** — **Labeled** actions aligned with [`evidence-export-contract.md`](./evidence-export-contract.md): which **export_kind**(s) apply given current **`subject_ref`** (policy dossier, topology dossier, situation room, investigation workspace). **Not** implying compliance, signing, or tamper evidence. When a **topology object** is in scope, the WebUI **may** offer a **live pivot** to **`maintenance_evidence_workspace_v1`** (`view=maintenance-evidence-workspace`) — that assembly is **outside** **`briefing_export_bundle_v1`** and per-surface **`evidence_export_v1`** members unless a future export family explicitly adds it (not required in Phase 2 v1).
