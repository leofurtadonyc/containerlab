...
- **Verifier / tests:** **`verify-core-runtime.sh`** — shipped **`/assets/*.js`** must include **`service_dossier_v1`** and **`change_safety_case_v1`** alongside existing NOC / digest / briefing / replay / week **31** markers; repository **`pytest`** / **`vitest`** (service dossier, change safety case navigation, global search, overview NOC, evidence replay parse, downloads)
