...
**Future implementation (not required by this document alone):** a dedicated read API such as **`GET /api/v1/services/{service_id}/dossier`** (name illustrative) returning **`ServiceDossierResponse`**—or an equivalent **WebUI-side** composition that **strictly** mirrors the same section contract and **does not** invent fields beyond nested APIs. Until implemented, this file is the **authoritative vocabulary** for dossier layout, reuse boundaries, and non-claims.
