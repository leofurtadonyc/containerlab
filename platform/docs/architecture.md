...
- **`/api/v1/actions`** implements **safe action workflow v1** per `docs/safe-action-workflow-contract-v1.md` — one bounded `policy_static_local_operator_intent_record_v1` slice with mandatory workflow, preview, validation, capability, and approval gates; outcome is a **platform Postgres** `policy_operator_intent_records` row (**not** device/controller configuration push, **not** a preview diff or validation verdict, **not** evidence replay)
