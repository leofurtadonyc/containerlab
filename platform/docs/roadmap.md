...
- topology still exposes the clearest remaining live truth gap, but the honest next question is no longer whether to add pairing vocabulary or partiality decomposition; those are already complete, and the narrower remaining step is whether the backend should expose node participation coverage inside the inferred slice from the evidence already in hand, while policy remains more blocked by absent derivable per-policy source detail in the current lab
