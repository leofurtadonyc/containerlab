...
- **Operator narrative alignment:** **`deployment-runbook.md`**, **`data-flows.md`**, **`production-readiness-assessment.md`**, and this roadmap describe the **same** bounded envelope: **`conditionally_ready_with_explicit_limits`**, same-workspace recovery only (host-backed data preserved), **product-owned** persisted history (including rich **`/api/v1/devices`** **`history`**) versus Grafana **current-metrics** and bounded **`inventory_snapshots`** table mirrors on **`app-api`** **`/metrics`**, conditional **`verify-core-runtime`** behavior when persisted rows exist or not, **Readiness** evaluation-sample versus **persisted-snapshot** language where relevant, and explicit **no** workflow, **no** dry-run, **no** validation-engine claims
...
Recommendation: preserve the week **16–19** checkpoint narrative (including **week 19** inventory-history API, product, verifier, metrics, and doc alignment) and continue bounded Phase 2 product deepening. Do not reopen the completed topology-history, policy-history, history-baseline, restart-drill, week 18–19 inventory-history, readiness-language, or Grafana-scaffold work without new evidence. Prefer the written decision in **`platform/docs/decisions/ADR-0001-next-bounded-truth-depth-slice.md`** before opening broad implementation in topology, policy, or persistence themes.
