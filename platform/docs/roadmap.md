...
- **Operator narrative alignment:** **`deployment-runbook.md`**, **`data-flows.md`**, **`production-readiness-assessment.md`**, and this roadmap describe the **same** bounded envelope: **`conditionally_ready_with_explicit_limits`**, same-workspace recovery only (host-backed data preserved), **product-owned** persisted history versus Grafana **current-metrics** mirrors, conditional **`verify-core-runtime`** behavior when persisted rows exist or not, **Readiness** evaluation-sample versus **persisted-snapshot** language where relevant, and explicit **no** workflow, **no** dry-run, **no** validation-engine claims
