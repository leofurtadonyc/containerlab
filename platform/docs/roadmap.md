...
- **Week 27 Monday task 01 (path-analysis contract and gap audit):** **`path-analysis-contract.md`**, **`ADR-0002`**, and **`schemas/path_analysis.py`** define **`path_analysis_phase2_v1`**—bounded read-only vocabulary (policy-anchored subject, intended/observed hints, candidate-path summaries, evidence attribution, freshness, truth-alignment posture, caveats, explicit non-claims); gap audit documents existing inventory/topology/policy/platform/ODL evidence vs missing dataplane/TE/controller path truth; repository **`pytest`** locks literals—**no** new HTTP route, **no** workflow or validation semantics; **`conditionally_ready_with_explicit_limits`** unchanged
