...
It reflects the **week 16–20 operator-truth envelope** (week **19** deepens **devices/inventory** persisted history and observability alignment; week **20** deepens **policies** persisted-history contracts, **`verify-core-runtime`** policy **`history`** assertions when gates match, **`app-api`** **`/metrics`** **`policy_snapshots`** table gauges, and cross-doc alignment with the proven Nokia **`static_local`** boundary—without changing the Phase 2 stop line): **conditional** preserved-baseline checks in
