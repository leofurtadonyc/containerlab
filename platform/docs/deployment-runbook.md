...
- `app-web` startup-contract readiness, static HTTP availability, `/api` proxy reachability to `app-api`, and (week **29–30** structural) shipped **`/assets/*.js`** references from **`/`** must include **`noc_cockpit_v1`**, **`overview_mode`**, **`cross_domain_delta_digest_v1`**, **`operator_briefing_workspace_v1`**, **`briefing_export_bundle_v1`**, **`evidence_replay_viewer_v1`**, **`noc-cockpit-strategic-pivots`**, and **`Evidence replay (frozen file)`**, **`Impact report hub`**, **`service_explorer_v1`**, **`policy_explainability_workspace_v1`** substrings somewhere in the downloaded scripts—proves the packaged WebUI still carries NOC cockpit, delta digest, operator briefing, briefing-bundle export, evidence replay, cockpit **2.0** strategic pivots, global-search week **30** footer markers, week **31** impact-report pivots, and week **31** service/explainability contract strings; **not** a substitute for repository **`vitest`**
