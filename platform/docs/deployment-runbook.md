...
- `app-web` startup-contract readiness, static HTTP availability, `/api` proxy reachability to `app-api`, and (week **29–34** structural) shipped **`/assets/*.js`** references from **`/`** must include **`noc_cockpit_v1`**, **`overview_mode`**, **`cross_domain_delta_digest_v1`**, **`operator_briefing_workspace_v1`**, **`briefing_export_bundle_v1`**, **`evidence_replay_viewer_v1`**, **`noc-cockpit-strategic-pivots`**, and **`Evidence replay (frozen file)`**, **`Impact report hub`**, **`service_explorer_v1`**, **`service_dossier_v1`**, **`policy_explainability_workspace_v1`**, **`path_explorer_v1`**, **`service_impact_workspace_v1`**, **`change_safety_case_v1`** substrings somewhere in the downloaded scripts—proves the packaged WebUI still carries NOC cockpit, delta digest, operator briefing, briefing-bundle export, evidence replay, cockpit **2.0** strategic pivots, global-search week **30** footer markers, week **31** impact-report pivots, week **31** service/explainability contract strings, week **34** Path Explorer + Service Impact Workspace contract ids, and week **32** service dossier + change safety case **contract-id strings** in the bundle; live **`GET`** proof of **Service Dossier** and **Change Safety Case** report routes are **separate** `app-api` branches when **`python3`** samples **`policy_id`**, **`service_id`**, and **`node_id`** (see following bullets and **`docs/week-32-verifier-parity-contract.md`**); **not** a substitute for repository **`vitest`**
