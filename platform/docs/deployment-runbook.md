...
It reflects the **week 16–22 operator-truth envelope** (week **19** deepens **devices/inventory** persisted history and observability alignment; week **20** deepens **policies** persisted-history contracts, **`verify-core-runtime`** policy **`history`** assertions when gates match, **`app-api`** **`/metrics`** **`policy_snapshots`** table gauges, and cross-doc alignment with the proven Nokia **`static_local`** boundary; week **22** documents bounded **read-side query ergonomics**—optional **`limit`**, **`history_recent_limit`**, **`sync_runs_limit`**, and **`readiness_snapshot_history_limit`** (audit only) with honest **`read_side_query`** echo, plus structural verifier sampling of optional bounded query strings on workflow-history and audit-history—**without** changing the Phase 2 stop line): **conditional** preserved-baseline checks in
