...
- `app-web` startup-contract readiness, static HTTP availability, `/api` proxy reachability to `app-api`, and (week **29–30** structural) shipped **`/assets/*.js`** references from **`/`** must include **`noc_cockpit_v1`**, **`overview_mode`**, **`cross_domain_delta_digest_v1`**, **`operator_briefing_workspace_v1`**, **`briefing_export_bundle_v1`**, and **`evidence_replay_viewer_v1`** substrings somewhere in the downloaded scripts—proves the packaged WebUI still carries NOC cockpit, delta digest, operator briefing, briefing-bundle export, and evidence replay composition markers; **not** a substitute for repository **`vitest`**
