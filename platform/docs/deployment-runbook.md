...
- **path analysis, topology-related-policies, and degraded-policy v1 (week 27, structural):** when **`python3`** is on **`PATH`**, the script parses the default **`/api/v1/policies`** and **`/api/v1/topology`** compact JSON to sample the first **`items[].policy_id`** and first **`topology.nodes[].node_id`**. When at least one policy exists, it **`GET`**s **`/api/v1/policies/{policy_id}/path-analysis`** and asserts **`path_analysis_phase2_v1`**, **`anchor_kind":"policy`**, and **`truth_alignment`**—plus **`contract_id":"degraded_policy_v1"`** on the policies list payload. When at least one topology node exists, it **`GET`**s **`/api/v1/topology/objects/{node_id}/related-policies`** and asserts **`object_kind":"node`**, matching **`object_id`**, **`derivation_summary`**, and an **`items`** array. If **`python3`** is missing, or the relevant list is empty, the script emits a **notice** and skips the corresponding checks—**not** a failure on an honest empty inventory
