...
- **Evidence consistency summary** (`evidence_consistency_summary_v1`) — cross-domain alignment/tension hints from existing read APIs; **`verify-core-runtime.sh`** checks compact **`GET /api/v1/evidence-consistency/summary`** JSON substrings and that shipped **`/assets/*.js`** still contains **`evidence_consistency_summary_v1`** (structural wiring only—not validation engines).

## Operator-visible contract boundaries (week 36 maintenance lane)

- **Maintenance evidence workspace** (`maintenance_evidence_workspace_v1`, **`GET /api/v1/maintenance-evidence-workspace`**) — composed read-only assembly over **Maintenance Preview**–shaped subjects plus nested dossier / timeline / delta / change-safety bodies where assembly succeeds—**not** **`evidence_export_v1`**, **not** **`impact_report_v1`** communication packaging alone, **not** maintenance approval or simulation.

## Operator-visible contract boundaries (week 37 stability lane)

- **Operational stability summary** (`operational_stability_summary_v1`, **`GET /api/v1/stability/summary`**) — bounded steadiness / churn / recurrence hints from existing read APIs—**not** prediction, unified health score, or **`evidence_consistency_summary_v1`** alignment scoring; **`verify-core-runtime.sh`** checks compact JSON and **`operational_stability_summary_v1`** in shipped **`/assets/*.js`**.
- **Topology object / service stability profiles** (`topology_object_stability_profile_v1`, **`GET /api/v1/topology/objects/{object_id}/stability-profile`**; `service_stability_profile_v1`, **`GET /api/v1/services/{service_id}/stability-profile`**) — per-subject stability lenses—**not** substitutes for dossier, timeline, delta, or Explorer detail JSON; **`verify-core-runtime.sh`** **`GET`**s these routes when **`python3`** samples **`node_id`** and **`service_id`** (same gates as week **27–36** dossier branches).

Authoritative contract clauses remain in **`change-safety-case-contract.md`**, **`impact-report-contract.md`**, **`evidence-export-contract.md`**, **`evidence-replay-viewer-contract.md`**, **`service-explorer-contract.md`**, **`service-dossier-contract.md`**, **`path-explorer-contract.md`**, **`service-impact-workspace-contract.md`**, **`evidence-consistency-summary-contract.md`**, **`maintenance-evidence-workspace-contract.md`**, **`topology-object-evidence-timeline-contract.md`**, **`topology-object-evidence-delta-contract.md`**, **`operational-stability-summary-contract.md`**, **`topology-object-stability-profile-contract.md`**, and **`service-stability-profile-contract.md`**. Scheduling context: [`week-33-bounded-next-slice-recommendation.md`](./week-33-bounded-next-slice-recommendation.md).
...
- `app-web` startup-contract readiness, static HTTP availability, `/api` proxy reachability to `app-api`, and (week **29–37** structural) shipped **`/assets/*.js`** references from **`/`** must include **`noc_cockpit_v1`**, **`overview_mode`**, **`cross_domain_delta_digest_v1`**, **`operator_briefing_workspace_v1`**, **`briefing_export_bundle_v1`**, **`evidence_replay_viewer_v1`**, **`noc-cockpit-strategic-pivots`**, and **`Evidence replay (frozen file)`**, **`Impact report hub`**, **`service_explorer_v1`**, **`service_dossier_v1`**, **`policy_explainability_workspace_v1`**, **`path_explorer_v1`**, **`service_impact_workspace_v1`**, **`change_safety_case_v1`**, **`evidence_consistency_summary_v1`**, **`maintenance_evidence_workspace_v1`**, **`operational_stability_summary_v1`** substrings somewhere in the downloaded scripts—proves the packaged WebUI still carries NOC cockpit, delta digest, operator briefing, briefing-bundle export, evidence replay, cockpit **2.0** strategic pivots, global-search week **30** footer markers, week **31** impact-report pivots, week **31** service/explainability contract strings, week **34** Path Explorer + Service Impact Workspace contract ids, week **32** service dossier + change safety case **contract-id strings**, week **35** evidence consistency workspace markers, week **36** maintenance evidence workspace contract text, and week **37** stability workspace markers in the bundle; live **`GET`** proof of **Service Dossier** and **Change Safety Case** report routes are **separate** `app-api` branches when **`python3`** samples **`policy_id`**, **`service_id`**, and **`node_id`** (see following bullets and **`docs/week-32-verifier-parity-contract.md`**); **not** a substitute for repository **`vitest`**
...
- **evidence export v1 (week 29, structural):** compact JSON **`GET`**s to **`/api/v1/exports/situation-room/summary`** and **`/api/v1/exports/investigation-workspace/summary`** must include **`evidence_export_v1`**, **`export_kind`**, **`explicit_non_claims`**, and nested payloads (**`evidence_pack_phase2_v1`** / **`investigation_workspace_phase2_v1`**); a bounded **`GET`** to **`/api/v1/exports/investigation-workspace/summary?sync_runs_limit=10`** must echo **`sync_runs_limit":10`** in **`subject_ref`**; when **`python3`** samples the first policy id and first topology node id (same gates as week **27–28** dossier checks), additional **`GET`**s to **`/api/v1/exports/policies/{policy_id}/dossier`** and **`/api/v1/exports/topology-objects/{node_id}/dossier`** must include the envelope plus nested **`policy_dossier_v1`** / **`topology_object_dossier_v1`**—proves export routes remain wired; **not** compliance artifacts, tamper evidence, or operational authority (the verifier checks JSON substrings only)
- **briefing export bundle v1 (week 30, structural):** a compact JSON **`GET`** to **`/api/v1/exports/operator-briefing?sync_runs_limit=10`** must include **`briefing_export_bundle_v1`**, **`bundle_members`**, and **`briefing_subject`**—proves the multi-member briefing archive route remains wired; **not** a substitute for replaying **`evidence_export_v1`** in **Evidence replay** (bundle root is a different contract than single-export replay); repository **`vitest`** asserts **`parseEvidenceExportJson`** rejects a raw **`briefing_export_bundle_v1`** root (operators use per-member JSON for replay)
- **Impact Report v1 (week 31, structural):** when **`python3`** samples the first policy id and first topology node id (same gates as week **27–28** dossier checks), compact JSON **`GET`**s to **`/api/v1/reports/policy-impact`** and **`/api/v1/reports/maintenance-impact`** (**`format=json`**) must include **`impact_report_v1`** and the matching **`report_context`**—proves report routes remain wired; **not** **`evidence_export_v1`** replay (different envelope; **Evidence replay** expects exports from **`GET /api/v1/exports/...`**); repository **`vitest`** rejects **`impact_report_v1`** in **`parseEvidenceExportJson`**
- **Service Dossier v1 (week 32 / week 33, structural):** when **`python3`** is available and **`GET /api/v1/services`** returns a non-empty **`items`** list, the script samples the first **`service_id`**, URL-encodes it, and **`GET`**s **`/api/v1/services/{service_id}/dossier`**—compact JSON must include **`service_dossier_v1`**, **`service_explorer_detail`**, **`merged_caveats`**, and **`source_contract_ids`** substrings; the same branch also **`GET`**s **`/api/v1/services/{service_id}/stability-profile`** (**`service_stability_profile_v1`**, week **37**) when **`items`** is non-empty; **not** assembly logic, **not** every possible **`service_id`**; skipped with a **notice** when **`python3`** is missing or **`items`** is empty—see **`docs/week-32-verifier-parity-contract.md`**
- **Change Safety Case v1 (week 32 / week 33, structural):** when **`python3`** samples the first **`policy_id`**, first **`service_id`**, and first **`node_id`** (same gates as **Impact Report** and **Service Dossier** branches), compact JSON **`GET`**s to **`/api/v1/reports/change-safety-case/policy`**, **`…/service`**, and **`…/maintenance`** (**`format=json`**) must include **`change_safety_case_v1`** and the matching **`safety_case_context`** (**`policy_change_safety`** / **`service_change_safety`** / **`topology_change_safety`**); **not** **`evidence_export_v1`** replay (different envelope; **Evidence replay** expects exports from **`GET /api/v1/exports/...`**); skipped with **notices** when the corresponding list is empty—see **`docs/week-32-verifier-parity-contract.md`**
- **Operational stability + stability profiles (week 37, structural):** compact JSON **`GET`** to **`/api/v1/stability/summary?sync_runs_limit=10`** must include **`operational_stability_summary_v1`**, **`operational_stability_posture`**, **`rows`**, **`sync_runs_limit_applied":10`**, and **`safety_framing`**; when **`python3`** samples the first topology **`node_id`** and first **`service_id`**, **`GET`**s to **`/api/v1/topology/objects/{node_id}/stability-profile`** and **`/api/v1/services/{service_id}/stability-profile`** must include **`topology_object_stability_profile_v1`** / **`service_stability_profile_v1`** and matching **`object_id`** / **`service_id`** echoes—proves routes remain wired; **not** business-logic validation. Shipped **`/assets/*.js`** must include **`operational_stability_summary_v1`** alongside prior week bundle markers; repository **`vitest`** **`week37-verifier-bundle-markers.test.ts`** pins source markers—**not** a substitute for repository **`pytest`** (**`test_operational_stability_summary.py`**, **`test_topology_object_stability_profile.py`**, **`test_service_stability_profile.py`**) or for interpreting stability semantics
- **Maintenance window workspace + handoff + WebUI `mww_subject` (week 38, structural):** when **`python3`** samples the first topology **`node_id`**, compact JSON **`GET`**s to **`/api/v1/maintenance-window-workspace?subject={url-encoded node:…}&sync_runs_limit=10&preview_context=planning_window`** must include **`maintenance_window_workspace_v1`** and **`deduped_affected_services`**; **`GET`** to **`/api/v1/exports/maintenance-window-handoff?subject=…&sync_runs_limit=10`** must include **`maintenance_window_handoff_v1`** and nested **`maintenance_window_workspace_v1`**—proves multi-subject workspace and distinct handoff export remain wired; **not** **`evidence_export_v1`**, **not** workflow approval. Shipped **`/assets/*.js`** must include **`mww_subject`** (shell query state for **`view=maintenance-window-workspace`**) alongside **`maintenance_window_workspace_v1`**; repository **`vitest`** **`week38-verifier-bundle-markers.test.ts`** pins **`mww_subject`** / **`/api/v1/maintenance-window-workspace`** / NOC + global-search pivot strings—**structural parity only**
- **cross-domain delta digest + operator briefing read APIs (week 30, structural):** compact JSON **`GET`** to **`/api/v1/delta-digest?sync_runs_limit=10`** must include **`cross_domain_delta_digest_v1`** and **`sections`**; **`GET`** to **`/api/v1/operator-briefing?sync_runs_limit=10`** must include **`operator_briefing_workspace_v1`** and **`section_meta`**—proves the digest and composed briefing routes remain wired end to end; **not** a substitute for repository **`pytest`** (**`test_delta_digest.py`**, **`test_operator_briefing.py`**) or for interpreting assembly semantics
- **path analysis, topology-related-policies, failure-impact, policy evidence timeline+delta, degraded-policy v1, and topology risk summary (week 27–28, structural):** the script always **`GET`**s **`/api/v1/topology/risk-summary`** and asserts **`topology_risk_summary_v1`** plus **`ranked_objects`**. When **`python3`** is on **`PATH`**, the script parses the default **`/api/v1/policies`** and **`/api/v1/topology`** compact JSON to sample the first **`items[].policy_id`** and first **`topology.nodes[].node_id`**. When at least one policy exists, it **`GET`**s **`/api/v1/policies/{policy_id}/path-analysis`** and asserts **`path_analysis_phase2_v1`**, **`anchor_kind":"policy`**, and **`truth_alignment`**; **`GET`**s **`/api/v1/policies/{policy_id}/evidence-timeline`** with **`policy_evidence_timeline_v1`**; **`GET`**s **`/api/v1/policies/{policy_id}/evidence-delta`** with **`policy_evidence_delta_v1`** and **`comparison_status`**—plus **`contract_id":"degraded_policy_v1"`** on the policies list payload. When at least one topology node exists, it **`GET`**s **`/api/v1/topology/objects/{node_id}/related-policies`** and asserts **`object_kind":"node`**, matching **`object_id`**, **`derivation_summary`**, and an **`items`** array; **`GET`**s **`/api/v1/topology/objects/{node_id}/failure-impact`** with **`failure_impact_v1`** and **`subject`**. If **`python3`** is missing, or the relevant list is empty, the script emits a **notice** and skips the corresponding checks—**not** a failure on an honest empty inventory
- **topology object dossier, topology object evidence timeline/delta, topology object stability profile, maintenance preview, and maintenance evidence workspace (week 29–37 structural extensions):** when a **`node_id`** is sampled, the script also **`GET`**s **`/api/v1/topology/objects/{node_id}/dossier`** (**`topology_object_dossier_v1`**), **`/evidence-timeline`** (**`topology_object_evidence_timeline_v1`**), **`/evidence-delta`** (**`topology_object_evidence_delta_v1`**), **`/stability-profile`** (**`topology_object_stability_profile_v1`**), **`/api/v1/maintenance-preview`** (aligned **`maintenance_preview_v1`**), **`/api/v1/maintenance-evidence-workspace`** (**`maintenance_evidence_workspace_v1`** with nested **`maintenance_preview_v1`** and **`topology_change_safety`** framing), plus maintenance-shaped **impact report** and **change safety case** routes in the same **`node_id`** branch—proves bounded topology/maintenance read assemblies remain wired; **not** business-logic validation, **not** approval or simulation truth
