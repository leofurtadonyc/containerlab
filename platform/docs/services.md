...
- **validation engine v1:** **`/api/v1/validations`** — durable, capability-gated **policy read-model observability** (`validation-result-contract-v1.md`, `decisions/ADR-0004-validation-engine-v1.md`); **not** preview/diff semantics, **not** execution or approval authority
- **week 28:** **`GET /api/v1/topology/objects/{object_id}/failure-impact`** (**`failure_impact_v1`**, `failure-impact-contract.md`) rolls up related-policy counts, **`degraded_policy_v1`** posture breakdown scoped to that related set, and path-analysis support tallies from existing read-side evidence—**not** blast-radius simulation, **not** dependency truth; **WebUI** **Topology** exposes **`TopologyFailureImpactPanel`** beside related policies when a node or link is selected; **`GET /api/v1/policies/{policy_id}/evidence-timeline`** (**`policy_evidence_timeline_v1`**, `policy-evidence-timeline-contract.md`) orders bounded policy-scoped anchors from inventory, history, path-analysis, and degraded v1—**not** a unified forensic chronology or validation engine; **WebUI** **Policies** **`PolicyEvidenceTimelinePanel`** (selected policy) consumes the same contract beside topology-impact and path-analysis—read-only assembly, honest sparse copy; **`GET /api/v1/policies/{policy_id}/evidence-delta`** (**`policy_evidence_delta_v1`**, `policy-evidence-delta-contract.md`) compares the current normalized inventory row to the **previous** persisted snapshot when policy history is **`comparison_ready`**—**not** drift truth, **not** config diff authority; **WebUI** **Policies** **`PolicyEvidenceDeltaPanel`** surfaces the same API after the evidence timeline—honest no-anchor and gap copy; client-only URL hints **`policy_evidence_timeline_focus`** / **`policy_evidence_delta_focus`** scroll to and briefly emphasize those panels (not app-api contracts); **`GET /api/v1/topology/risk-summary`** (**`topology_risk_summary_v1`**, `topology-risk-summary-contract.md`) ranks every topology node and link using the same related-policy and degraded v1 counting rules—**not** SLA or traffic risk, **not** global policy health; **WebUI** **Overview** and **Topology** **`TopologyRiskAttentionPanel`** consume that API with bounded drillthrough; **WebUI** **Overview** **`OperatorWorkspaceEntry`** composes read-only navigation into risk attention (in-page anchor + **`view=topology`**), failure impact on the first available node, and policy timeline/delta entry points from the first policy row when present—composition and shell navigation only, **not** new backend routes
