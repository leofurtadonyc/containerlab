...
- **week 28:** **`GET /api/v1/topology/objects/{object_id}/failure-impact`** (**`failure_impact_v1`**, `failure-impact-contract.md`) rolls up related-policy counts, **`degraded_policy_v1`** posture breakdown scoped to that related set, and path-analysis support tallies from existing read-side evidence—**not** blast-radius simulation, **not** dependency truth
