...
- **week 28:** **`GET /api/v1/topology/objects/{object_id}/failure-impact`** (**`failure_impact_v1`**, `failure-impact-contract.md`) rolls up related-policy counts, **`degraded_policy_v1`** posture breakdown scoped to that related set, and path-analysis support tallies from existing read-side evidence—**not** blast-radius simulation, **not** dependency truth; **WebUI** **Topology** exposes **`TopologyFailureImpactPanel`** beside related policies when a node or link is selected; **`policy-evidence-timeline-contract.md`** defines a future **`policy_evidence_timeline_v1`** per-policy evidence ordering from existing read-side timestamps—**not** a unified forensic chronology or validation engine (contract-only until implemented)
