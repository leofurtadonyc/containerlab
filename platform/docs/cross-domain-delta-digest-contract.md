...
**Implementation posture (v1):** May ship first as a **WebUI composition** (e.g. Overview region or dedicated view) that calls **only** documented **`GET /api/v1/...`** routes, or as a thin read-only **`GET /api/v1/.../cross-domain-delta-digest`** assembly that **composes existing services**—this file defines **what the digest may claim**, **section order**, and **honesty limits**, not a mandatory backend shape before a UI exists.
