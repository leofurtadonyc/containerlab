...
- the SR policy dashboard **Persisted Policy Sync Evidence** row now also includes **`platform_app_api_policy_snapshots_persisted_total`** next to recent policy sync-run counts and compares policy sync age with **`platform_app_api_policy_snapshot_latest_persisted_at_seconds`** (Postgres `policy_snapshots` table chronology only—not **`/api/v1/policies`** **`history`**)
