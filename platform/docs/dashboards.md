...
- **week 28 product versus observability:** **failure-impact** (`failure-impact-contract.md`), **topology risk summary** (`topology-risk-summary-contract.md`), **policy evidence timeline** (`policy-evidence-timeline-contract.md`), and **policy evidence delta** (`policy-evidence-delta-contract.md`), plus **Overview** composition (**`OperatorWorkspaceEntry`**, navigation only), are **app-api + WebUI** read surfaces. Grafana does **not** implement ranking, failure-impact rollups, policy timeline/delta semantics, or operator-workspace navigation; panels continue to mirror **metrics and bounded table-age or sync signals** only—not product contract semantics
