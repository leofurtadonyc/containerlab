...
**Implementation posture (v1):** **`GET /api/v1/operator-search?q=...`** (**`OperatorSearchResponse`**, **`schemas/operator_search.py`**, **`services/operator_search.py`**, route on **`routers/operator_search.py`**) aggregates the same normalized list fields as **`GET /api/v1/devices`**, **`GET /api/v1/policies`**, **`GET /api/v1/topology`**, and **`GET /api/v1/capabilities`**—grouped, capped per family, with **`ranking_basis`**, **`match_reason`**, and recommended **`pivot`** targets. **WebUI:** **`GlobalOperatorSearch`** (**`app-web`**, **`features/global-search/global-operator-search.tsx`**) calls the same endpoint from the shell and pivots using existing dossier/detail navigation helpers. Client-only **`global_search_q`** echoes the effective query on deep links (with **`policy_dossier_entry`**, **`dossier_source`**, **`inv_from=global_search`**, **`readiness_capability_feature`**, etc., as applicable). Secondary actions may open **Delta digest** (cross-domain assembly with the same **`sync_runs_limit`** as other pivots), **Operator briefing**, **Investigation**, **Readiness** (capability rows), **Situation room**, or **Evidence replay** — the last is **only** a route to the frozen **`evidence_export_v1`** import surface (**`evidence_replay_viewer_v1`**); it does **not** imply inventory hits match a loaded file. This file remains the honesty contract for both API and UI.
