...
- **Deeper topology truth v1:** **`GET /api/v1/topology/truth`** serves **`topology_truth_v1`** per **`deeper-topology-truth-contract-v1.md`**, **`ADR-0007`**, **`ADR-0010`**, and **`ADR-0011`** — merges gNMI-normalized topology, LLDP physical adjacency evidence, device-native OSPF/IS-IS adjacency evidence, and optional ODL **`network-topology`** enrichment; **`platform_app.topology_truth_snapshots`** optional persistence; metrics **`platform_app_api_topology_truth_*`**; **WebUI** **Topology** on-demand **Load merged truth** panel—**not** dataplane path truth, **not** sole ODL authority, **not** TE resolution
- **Controller southbound session truth v2:** **`GET /api/v1/controller/evidence`** serves **`controller_southbound_session_truth_v2`** per **`controller-southbound-session-truth-v2.md`** / **`ADR-0009`** — distinct **BGP-LS / PCEP / NETCONF** lanes with **session posture**, **evidence strength**, **derivation mode**, **protocol/object visibility**, **`yang_module_catalog_count`**; native RESTCONF probes (`yang_module_catalog`, `native_session_probes`) plus topology/heuristic fallbacks when stronger paths are absent; **`platform_app.controller_evidence_snapshots`** optional persistence (lane JSON includes v2 contract id); metrics **`platform_app_api_controller_evidence_*`** including per-lane session/evidence counters; **WebUI** **Platform Health** on-demand **Load controller evidence**—**not** dataplane path truth, **not** sole ODL authority, **not** wire-level proof for every southbound session on every device (v1 reference: **`ADR-0008`**)
...
- The packaged runtime currently sets a shared collector budget of `5s` with a larger `8s` budget for topology because the normalized topology snapshot is slower than inventory and policy in the present lab footprint. Treat those values as runtime contract tuning, not as arbitrary scaffolding.
...
- the Overview WebUI now starts its core independent read-only queries in parallel instead of serially gating topology after devices, policies after topology, and platform status after policies; keep that posture unless a later contract introduces a real dependency between those slices
- `/api/v1/platform/status` now assembles its three collector-backed read paths in parallel before composing the response; heavier read-only assemblies such as delta digest, investigation workspace, situation room, and operator briefing also fan out independent nested builders in parallel so backend composition does not add avoidable latency on top of collector time
...
- **Validation engine v1:** **`validation_requests`** and **`validation_events`** persist bounded **policy read-model observability** verdicts; **`GET` / `POST` `/api/v1/validations`** — see **`validation-result-contract-v1.md`** / **`ADR-0004-validation-engine-v1.md`** — **not** actuation, **not** approval policy, **not** preview or evidence deltas
- **Deeper topology truth v1 (read-side merge):** optional **`platform_app.topology_truth_snapshots`** stores bounded merged-view payloads for inspection; live **`GET /api/v1/topology/truth`** remains the product read—see **`deeper-topology-truth-contract-v1.md`** / **`ADR-0007`**
- **Controller southbound evidence v1:** optional **`platform_app.controller_evidence_snapshots`** stores lane summaries for inspection; live **`GET /api/v1/controller/evidence`** remains the product read—see **`controller-southbound-evidence-v1.md`** / **`ADR-0008`**
- **Safe action workflow v1:** **`safe_actions`**, **`safe_action_events`**, and **`policy_operator_intent_records`** persist **one bounded execute path**; **`GET` / `POST` `/api/v1/actions`** (approve / execute / cancel / timeline) — see **`safe-action-workflow-contract-v1.md`** / **`ADR-0005-safe-action-workflows-v1.md`** — prerequisite-gated against lifecycle + preview + validation; **platform Postgres overlay only**; **not** preview output, **not** validation verdict, **not** device/controller push
