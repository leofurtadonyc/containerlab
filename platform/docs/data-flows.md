...
- **Post-deploy structural checks (week 27):** when **`python3`** is available on the host running **`./scripts/verify-core-runtime.sh`**, the script samples the first policy id and first topology node id from the live compact **`/api/v1/policies`** and **`/api/v1/topology`** responses and asserts stable JSON substrings on **`GET /api/v1/policies/{policy_id}/path-analysis`** and **`GET /api/v1/topology/objects/{node_id}/related-policies`**, plus **`contract_id":"degraded_policy_v1"`** on policy items when the list is non-empty—**contract presence only**, not business-logic duplication; skipped with an honest **notice** when **`python3`** is missing or lists are empty (see **`deployment-runbook.md`**)
