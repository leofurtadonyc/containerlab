...
- **Path analysis (week 27 Monday task 01, contract only):** backend-owned vocabulary in **`schemas/path_analysis.py`** and **`path-analysis-contract.md`** / **`ADR-0002`** defines a future **read-only** policy-anchored path interpretation surface (intended vs observed hints, candidate-path summaries, freshness, caveats)—**not** dataplane forwarding truth, TE resolution, or controller path authority; **no** new HTTP route in this task
