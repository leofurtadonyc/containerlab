...
- **Post-deploy structural checks (week 27–28):** **`verify-core-runtime.sh`** always asserts **`GET /api/v1/topology/risk-summary`** includes **`topology_risk_summary_v1`** and **`ranked_objects`**. When **`python3`** is available, it samples the first policy id and first topology node id from the live compact **`/api/v1/policies`** and **`/api/v1/topology`** responses and asserts stable JSON substrings on **`GET /api/v1/policies/{policy_id}/path-analysis`**, **`GET /api/v1/policies/{policy_id}/evidence-timeline`**, **`GET /api/v1/policies/{policy_id}/evidence-delta`** (including **`comparison_status`**), **`GET /api/v1/topology/objects/{node_id}/related-policies`**, and **`GET /api/v1/topology/objects/{node_id}/failure-impact`**, plus **`contract_id":"degraded_policy_v1"`** on policy items when the list is non-empty—**contract presence only**, not business-logic duplication; skipped with an honest **notice** when **`python3`** is missing or lists are empty (see **`deployment-runbook.md`**)
