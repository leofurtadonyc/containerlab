...
- **Investigation workspace (week 25):** **`GET /api/v1/investigation-workspace/context`** assembles nested **existing** responses (change-intelligence **`recent-summary`**, **`/api/v1/platform/status`**, **`/api/v1/capabilities`**) per **`investigation-workspace-contract.md`** / **`schemas/investigation_workspace.py`** / **`services/investigation_workspace.py`**—interpretation support only; **not** validation, execution, or safe-to-change authority; **`verify-core-runtime.sh`** includes structural checks on the live JSON; **WebUI** **Overview** exposes a bounded **Open investigation workspace** entry (same **`sync_runs_limit`** window as the Overview recent-change summary) and **`view=investigation`** renders a dedicated **`InvestigationWorkspaceProduct`** read-only layout (safety framing, domain-level recent-change rows, platform read paths, capabilities excerpt)—**not** workflow or approval semantics
